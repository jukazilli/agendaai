...
| 10 | B-10 | Confirmar execucao e reconhecer receita | B-09 | PARCIAL | atendimento concluido gera reflexo financeiro | dashboard e relatorios agora leem receita reconhecida e entrada online aprovada a partir de um read model minimo no `api-rest`; falta `cash entry` persistido e conciliacao financeira |
| 11 | B-11 | Implementar carteira basica de clientes | B-07, B-10 | PARCIAL | cliente exibe historico e ultima visita | `admin-web` agora mostra historico basico, ultimo atendimento concluido, receita derivada, retorno por janela, buckets simples de retorno e recorrencia media; detalhe denso e CRM transacional continuam pendentes |
| 12 | B-12 | Implementar relatorios essenciais | B-10, B-11 | PARCIAL | agenda, receita e retorno com filtros basicos | `admin-web` agora possui rota dedicada de `relatorios` ligada a `GET /v1/admin/read-models/reports`, com filtros de periodo, servico e profissional, comparativo com periodo anterior, buckets de retorno e recorrencia basica; exportacao, `cash entry` persistido e cohort avancado seguem pendentes |
